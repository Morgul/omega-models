...
function Collection(name, root, writeToDisk, gzip, spaces)
{
...
Collection.prototype._writeToDisk = function()
{
    ...
    else
    {
        ...
        mkdirp(path.dirname(this.path), function(error)
        {
            ...
            var data = JSON.stringify(this.data, null, this.spaces);
...
Collection.prototype.remove = function(filter)
{
    ...
    {
        //TODO: Test this code; it had a bug in it that means it could never have worked!
        ...
        for(var idx = 0; idx < keys.length; idx++)
        {
            ...
            for(var ydx=0; ydx < subKeys.length; ydx++)
            {
...
SimpleBackend.prototype._getCollection = function(model, callback, final)
{
    ...
    else
    {
        try
        {
            collection = new Collection(collectionName, this.config.get('root', './db'), this.config.get('writeToDisk', true), this.config.get('gzip', false), this.config.get('spaces'));
