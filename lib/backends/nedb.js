...
NEDBBackend.prototype._getCollection = function(modelInst)
{
    ...
    {
        var autoload = !this.config.autoload && this.config.autoload !== false ? true : this.config.autoload;
        var dbPath = path.resolve(this.config.baseDir);
        ensurePath(dbPath);

        this.collections[name] = new Datastore({ filename:path.join(dbPath, name + '.nedb'), autoload: autoload });
    } // end if
...
NEDBBackend.prototype.findOneAndUpdate = function(model, filter, update, callback)
{
    callback = callback || function(){};
    var db = this._getCollection(model);
    db.update(filter, update, {}, function(error)
    {
        if(error)
        {
            callback(error);
...
NEDBBackend.prototype.update = function(model, filter, update, callback)
{
    callback = callback || function(){};
    var db = this._getCollection(model);
    db.update(filter, update, {}, callback);
}; // end update
