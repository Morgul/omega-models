...
Model.prototype.save = function(callback)
{
    ...
    try
    {
        ...
        {
            if(field.constructor.name != 'AutoIDField')
            {
...
//TODO: This is broken if there's more than one reference field. Should use async.js to sort out the needed callbacks.
Model.prototype.populate = function(callback)
{
    callback = callback || function(){};
    var self = this;
    ...
    {
        if(field.constructor.name == 'ReferenceField')
        {
            var value = self.$values[name];
            var RefModel = self.constructor.ns[field.opts.model];
            RefModel.findOne(value, function(error, model)
            {
                console.log("model:", model);

                if(!error && model)
                {
                    self.$values[name] = model;
                } // end if
